...
(function () {
  ...
  /* O usuário da sessão chega com os nomes do DTO do servidor, `roleTitle` e
     `initials`, mas as telas leem `role` e `ini`, que são os nomes usados no
     diretório de pessoas. Sem espelhar: o campo Cargo de Minha conta aparecia vazio
     e "Salvar alterações" quebrava em `f.role.trim()` de undefined, e o avatar do
     próprio usuário ficava sem iniciais em Minha conta e em Relatórios.
     Espelha em vez de renomear: `roleTitle` e `initials` continuam publicados (§8.3). */
  function normalizeUser(u) {
    if (!u) return null;
    var out = Object.assign({}, u, { role: u.role || u.roleTitle, ini: u.ini || u.initials });
